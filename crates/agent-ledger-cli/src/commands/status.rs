...
pub async fn run() -> anyhow::Result<()> {
    ...
    println!(
        ...
        snapshot.tokens.reported_tokens_total
    );
    println!(
        "reported_input_tokens_total: {}",
        snapshot.tokens.input_tokens_total
    );
    println!(
        "reported_output_tokens_total: {}",
        snapshot.tokens.output_tokens_total
    );
    println!(
        "reported_cached_tokens_total: {}",
        snapshot.tokens.cached_tokens_total
    );
    if let Some(aic_used) = snapshot.tokens.aic_used {
        println!("aic_used: {}", aic_used);
    }
    if let Some(aic_remaining) = snapshot.tokens.aic_remaining {
        println!("aic_remaining: {}", aic_remaining);
    }
    if let Some(aic_limit) = snapshot.tokens.aic_limit {
        println!("aic_limit: {}", aic_limit);
    }
